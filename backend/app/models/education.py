...
class EducationSummary(BaseModel):
    ...
    slug: str = Field(
        ..., description="URL-safe identifier matching the Firestore document ID."
    )
    title: str = Field(..., description="Article title.")
    category: EducationCategory = Field(..., description="Emission category covered.")
    read_time: int = Field(
        ..., ge=1, le=60, description="Estimated reading time in minutes."
    )
...
class EducationDetail(EducationSummary):
    ...
    content: str = Field(
        ...
        description=(
            ...
            "Rendered on the frontend with react-markdown."
        ),
    )
